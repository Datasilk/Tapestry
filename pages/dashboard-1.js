...
//duplicate interface 3 times to create small, medium, & large versions of the Tapestry UI
var interfaces = [
            {size:"sm", name:"Small"}];
            //{size:"med", name:"Medium"},
            //{size:"lg", name:"Large"}];
htm = ui.innerHTML;
for(var x = 0; x < interfaces.length; x++){
    addHtm('div',container, 
        replace(replace(htm
            ,'{{size}}',interfaces[x].size)
            , '{{name}}',interfaces[x].name)
    );
}
...
for(var x = 0; x < interfaces.length; x++){
    ...
    //generate file list
    var fileui = document.getElementById(interfaces[x].size + 'filelist_ui');
    var filecontainer = document.getElementById(interfaces[x].size + 'filelist');
